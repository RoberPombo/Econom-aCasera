...
import type { SummaryResult, TransactionRepository } from "../repositories/TransactionRepository";
...
class InMemoryTransactionRepository implements TransactionRepository {
  ...
  async getByYearAndMonth(year: number, month?: number): Promise<Transaction[]> {
    return this.items.filter((t) => t.year === year && (month === undefined || t.month === month));
  }
  ...
  async getFiltered(filters: TransactionFilters): Promise<Transaction[]> {
    return this.items.filter((t) => {
      if (filters.period.mode === "month") {
        if (t.year !== filters.period.year || t.month !== filters.period.month) return false;
      } else {
      ...
      if (filters.categoryKeys.length && !filters.categoryKeys.includes(t.category)) return false;
      if (filters.personKeys.length && !filters.personKeys.includes(t.person)) return false;
  ...
  async getSummary(year: number, month?: number): Promise<SummaryResult> {
    return this.getSummaryFiltered(TransactionFilters.defaultMonth(year, month ?? 1));
  }
...
describe("GetTransactionsUseCase with filters", () => {
  const repo = new InMemoryTransactionRepository([
    tx({ date: "2026-08-01", type: "expense", category: "comida", concept: "Mercadona", amount: 40, person: "ana" }),
    tx({ date: "2026-08-15", type: "income", category: "nomina", concept: "Nómina agosto", amount: 1500, person: "ana" }),
    tx({ date: "2026-07-10", type: "expense", category: "transporte", concept: "Gasolina", amount: 60, person: "bob" }),
    tx({ date: "2025-08-01", type: "expense", category: "comida", concept: "Mercadona", amount: 30, person: "ana" }),
  ]);
  ...
  test("filters by month period", async () => {
    const result = await getTx.execute(TransactionFilters.defaultMonth(2026, 8));
  ...
  test("AND combines search, type and person across open range", async () => {
    ...
    expect(result).toHaveLength(2);
    expect(result.every((t) => t.concept.toLowerCase().includes("mercadona"))).toBe(true);
  });
